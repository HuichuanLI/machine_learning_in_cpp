...
/*   函数原型声明      */
void printp(double pheromone[2][STEP]);/*显示*/
void printmstep(int mstep[NOA][STEP]);

/*显示蚂蚁的行动*/
void walk(int cost[2][STEP], double pheromone[2][STEP], int mstep[NOA][STEP]);/*使蚂蚁步行*/
void update(int cost[2][STEP], double pheromone[2][STEP], int mstep[NOA][STEP]);/*更新信息素*/
double rand1();/*返回0～1实数的随机函数*/
...
void walk(int cost[2][STEP], double pheromone[2][STEP], int mstep[NOA][STEP]) {
    ...
    for (m = 0; m < NOA; ++m) {
        for (s = 0; s < STEP; ++s) {
            /*基于ε-greedy的行动选择*/
            if ((rand1() < EPSILON)
                || (abs(pheromone[0][s] - pheromone[1][s]) < 1e-9)) {/*随机行动*/
                mstep[m][s] = rand01();
            } else {/*基于信息素的选择*/
